import React from "react";


function Blogspost(){





return(

<section id="blogs">


<div class="hero bg-zinc-800 py-10 h-fill w-full">
  <div class="hero-content">
    <div class="min-h-screen w-fit ">
    <div data-aos="fade-up" data-aos-duration="1000" data-aos-delay="100">
    <h1 class="text-5xl font-bold text-center text-white">Blog Spot</h1>
    <h1 class=" text-center text-white py-6">Blogs to Inspire and Inform.</h1>
    <br/>
    </div>

<div data-aos="fade-up" data-aos-duration="500" data-aos-delay="100">
<div class="flex md:flex-col flex-col lg:flex-row w-full">

<div data-aos="zoom-in" data-aos-duration="500" data-aos-delay="200">

<div data-aos="fade-down" data-aos-duration="1000" data-aos-delay="300">
<div class="card bg-neutral shadow rounded-lg card-compact lg:w-80 sm:w-96 ">
  <figure><img src="https://www.hrkatha.com/wp-content/uploads/indian-flag-2644512_640.jpg" alt="OLQs" /></figure>
  
  <div class="card-body">
    <h2 class="card-title text-neutral-content text-white">Essential Requisite Qualities of an Officer</h2>
    <p class="text-black badge badge-secondary rounded-lg">Blogger</p>
    <div class="card-actions justify-end pt-6"> 
      <a href="https://shivansawasthi.blogspot.com/2024/04/essential-requisite-qualities-of-officer.html"><button class="btn btn-primary text-white">Open Blog<ion-icon name="open"></ion-icon></button></a>
    </div>
  </div>
</div>
</div>

</div>


</div>
</div>

    </div>
    </div>
  </div>


</section>


) ;

}

export default Blogspost ;