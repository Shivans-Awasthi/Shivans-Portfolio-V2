import React from "react";


function Blogspost(){





return(

<section id="blogs">


<div class="hero min-h-screen w-full">
<div class="glowing-sphere"></div>
  <div class="hero-content font-bold">
    <div class="min-h-screen w-fit ">
    <br/><br/><br/><br/>
    <div data-aos="fade-up" data-aos-duration="1000" data-aos-delay="100">
    <h1 class="text-4xl font-bold myname text-left text-black">Blog Spot</h1>
    <h1 class="text-xl text-left border-b-4 shadow-xl border-white text-black p-2">Blogs to Inspire and Inform.</h1>
    <br/>
    </div>
<br/><br/>

<div data-aos="fade-up" data-aos-duration="500" data-aos-delay="100">
<div class="flex md:flex-col flex-col lg:flex-row w-full">

<div data-aos="zoom-in" data-aos-duration="500" data-aos-delay="200">

<div data-aos="fade-down" data-aos-duration="1000" data-aos-delay="300">
<div class="card glass1 rounded card-compact lg:w-80 sm:w-96 00 shadow-xl ">
  <figure><img src="https://www.hrkatha.com/wp-content/uploads/indian-flag-2644512_640.jpg" alt="OLQs" /></figure>
  
  <div class="card-body">
    <h2 class="card-title text-neutral-content text-black">Essential Requisite Qualities of an Officer</h2>
    <p class="text-black">Blogger</p>
    <div class="card-actions justify-end">
      <a href="https://shivansawasthi.blogspot.com/2024/04/essential-requisite-qualities-of-officer.html"><button class="btn btn-outline bg-gray-200 text-black text-lg hover:bg-black hover:text-white rounded-none">Open Blog</button></a>
    </div>
  </div>
</div>
</div>

</div>


</div>
</div>

    </div>
    </div>
  </div>


</section>


) ;

}

export default Blogspost ;