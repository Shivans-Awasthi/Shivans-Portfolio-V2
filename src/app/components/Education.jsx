import React from "react";



function Education(){


return(

<section id="education" >

<div class="hero bg-zinc-950 h-fill py-10 w-full">  

<div class="hero-content">
<div class="text-left w-fit">
<br/><br/>

<div data-aos="fade-up" data-aos-duration="1000" data-aos-delay="100">
<h1 class="text-5xl font-bold text-center text-white"><ion-icon name="school"></ion-icon> Education</h1>
<h1 class=" text-white text-center py-6">Cultivating a lifelong love of learning through academic excellence and intellectual curiosity.</h1>

</div>


<ul class="timeline timeline-snap-icon max-md:timeline-compact timeline-vertical text-white ">
  <li>
    <div class="timeline-middle">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5 text-accent"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" /></svg>
    </div>
    <div class="timeline-start text-end mb-10">
    <div data-aos="fade-up" data-aos-duration="500" data-aos-delay="200"><time class="font-mono italic">2024</time></div>
    <div data-aos="fade-right" data-aos-duration="500" data-aos-delay="300"><div class="text-white font-bold">B.Tech Computer Science and System Engineering</div>
    <p class="text-neutral-500">Kalinga Institute of Industrial Technology(KIIT),<br/> Bhubaneshwar, Odisha, India</p>
      <p class="text-accent text-sm">CGPA: 8.36</p>
    </div>
    </div>
    <hr class="bg-accent"/>
  </li>
  <li>
    <hr />
    <div class="timeline-middle">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5 text-gray-500"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" /></svg>
    </div>
    <div class="timeline-end mb-10">
    <div data-aos="fade-up" data-aos-duration="500" data-aos-delay="300"><time class="font-mono italic">2019</time></div>
    <div data-aos="fade-left" data-aos-duration="500" data-aos-delay="300"><h1 class="font-bold text-white">Class XII- Higher Secondary Education(CBSE)</h1>
    <p class="text-neutral-500">Kendriya Vidyalaya No.1,<br/> Mumbai, Maharashtra, India</p>
      <p class="text-accent text-sm">Percentage: 85%</p>
    </div>
    </div>
    
    <hr class="bg-gray-500"/>
  </li>

</ul>
</div>
</div>
</div>


</section>


) ;

}

export default Education ;